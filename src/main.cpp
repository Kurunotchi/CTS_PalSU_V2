...
void logSimpsonOutputRow(const char* sheetId, float voltage, float current, float capacity);
...
void logPhaseStart(char slot, const String& statusText) {
  ...
  char sheetId[50]; float v=0, cap=0;
  INA226* sensor = nullptr;
  switch(slot) {
    case 'A': strcpy(sheetId, SHEET_ID_A); v=voltageA; cap=capacityA; sensor=&inaA; break;
    case 'B': strcpy(sheetId, SHEET_ID_B); v=voltageB; cap=capacityB; sensor=&inaB; break;
    case 'C': strcpy(sheetId, SHEET_ID_C); v=voltageC; cap=capacityC; sensor=&inaC; break;
    case 'D': strcpy(sheetId, SHEET_ID_D); v=voltageD; cap=capacityD; sensor=&inaD; break;
  }

  // 1. Log with 0 current to mark the start
  logSlotToSheet(sheetId, statusText, bn, 0, v, 0, cap);

  // 2. Wait for the relays to actually affect the circuit and the INA226 to read it
  delay(200);
  ...
  if (sensor != nullptr) {
    if (slot == 'A') {
      if (Acharge) v_read = sensor->readBusVoltage() * voltageCalAcharge;
      else if (Adischarge) v_read = sensor->readBusVoltage() * voltageCalAdischarge;
      else v_read = sensor->readBusVoltage() * voltageCalA;
      c_read = sensor->readShuntCurrent() * currentCalA;
    } else if (slot == 'B') {
      if (Bcharge) v_read = sensor->readBusVoltage() * voltageCalBcharge;
      else if (Bdischarge) v_read = sensor->readBusVoltage() * voltageCalBdischarge;
      else v_read = sensor->readBusVoltage() * voltageCalB;
      c_read = sensor->readShuntCurrent() * currentCalB;
    } else if (slot == 'C') {
      if (Ccharge) v_read = sensor->readBusVoltage() * voltageCalCcharge;
      else if (Cdischarge) v_read = sensor->readBusVoltage() * voltageCalCdischarge;
      else v_read = sensor->readBusVoltage() * voltageCalC;
      c_read = sensor->readShuntCurrent() * currentCalC;
    } else if (slot == 'D') {
      if (Dcharge) v_read = sensor->readBusVoltage() * voltageCalDcharge;
      else if (Ddischarge) v_read = sensor->readBusVoltage() * voltageCalDdischarge;
      else v_read = sensor->readBusVoltage() * voltageCalD;
      c_read = sensor->readShuntCurrent() * currentCalD;
    }

    if (v_read < 2.0) v_read = 0.0;
    if (c_read > -5.0 && c_read < 5.0) c_read = 0.0;
    c_read = abs(c_read);

    // Update the global state so immediate UI shows actual current
    if (slot == 'A') { voltageA = v_read; currentA = c_read; }
    else if (slot == 'B') { voltageB = v_read; currentB = c_read; }
    else if (slot == 'C') { voltageC = v_read; currentC = c_read; }
    else if (slot == 'D') { voltageD = v_read; currentD = c_read; }
  }

  // 3. Update local vars and log actual current
  float c = 0;
  switch (slot) {
    case 'A': v = voltageA; c = currentA; cap = capacityA; break;
    case 'B': v = voltageB; c = currentB; cap = capacityB; break;
    case 'C': v = voltageC; c = currentC; cap = capacityC; break;
    case 'D': v = voltageD; c = currentD; cap = capacityD; break;
  }
...
void handleStatus() {
  ...
  auto addSlotData = [&](char slot, int bn, unsigned long elapsed,
                          float v, float c, float cap,
                          bool charge, bool discharge, bool cycle,
                          int cycleCnt, int cycleTgt) {
    ...
    switch(slot) {
      case 'A': isComplete = cycleCompleteA; simpsonCap = capacityA; break;
      case 'B': isComplete = cycleCompleteB; simpsonCap = capacityB; break;
      case 'C': isComplete = cycleCompleteC; simpsonCap = capacityC; break;
      case 'D': isComplete = cycleCompleteD; simpsonCap = capacityD; break;
    }
    if (isComplete && simpsonCap > 0) {
      mode = String("Simpson Capacity ") + String(simpsonCap, 2) + String("mAh");
    } else if (cycle) {
      bool pauseActive = false;
      bool rechargePauseActive = false;
      switch(slot) {
        case 'A': pauseActive = AcyclePauseWait; rechargePauseActive = AcycleRechargePauseWait; break;
        case 'B': pauseActive = BcyclePauseWait; rechargePauseActive = BcycleRechargePauseWait; break;
        case 'C': pauseActive = CcyclePauseWait; rechargePauseActive = CcycleRechargePauseWait; break;
        case 'D': pauseActive = DcyclePauseWait; rechargePauseActive = DcycleRechargePauseWait; break;
      }
      if (pauseActive || rechargePauseActive) mode = "CYCLE - PAUSING";
      else if (charge)                        mode = "CYCLE - CHARGING";
...
void handleCommand() {
  ...
  if (error) { server.send(400, "text/plain", "Failed to parse JSON"); return; }
  ...
  if (cmd && slot) {
    ...
    switch(cmdChar) {
      ...
      case '4': // Set Battery Number
        if (doc["battery_number"] > 0) saveBattNum(slotChar, doc["battery_number"]);
        break;
      case '0': // Stop
...
void setup() {
  ...
  while (!GSheet.ready() && millis() - start < 10000) { Serial.print("."); delay(500); }
  if (GSheet.ready()) { Serial.println("\nGSheet Ready!"); gsheetReadyOnce = true; }
  else                { Serial.println("\nGSheet timeout. Will retry."); gsheetReadyOnce = false; }
...
void resetSimpsonTracking(char slot) {
  switch(slot) {
    case 'A':
      sampleCountA = 0; capacityTrackingA = false;
      prevCurrentA = 0; prevPrevCurrentA = 0;
      prevTimeA = 0; prevPrevTimeA = 0;
      break;
    case 'B':
      sampleCountB = 0; capacityTrackingB = false;
      prevCurrentB = 0; prevPrevCurrentB = 0;
      prevTimeB = 0; prevPrevTimeB = 0;
      break;
    case 'C':
      sampleCountC = 0; capacityTrackingC = false;
      prevCurrentC = 0; prevPrevCurrentC = 0;
      prevTimeC = 0; prevPrevTimeC = 0;
      break;
    case 'D':
      sampleCountD = 0; capacityTrackingD = false;
      prevCurrentD = 0; prevPrevCurrentD = 0;
      prevTimeD = 0; prevPrevTimeD = 0;
      break;
  }
}
...
void calculateSimpsonCapacity(char slot, float current, unsigned long currentTime) {
  ...
  switch (slot) {
    case 'A': capacity=&capacityA; prevCurrent=&prevCurrentA; prevPrevCurrent=&prevPrevCurrentA;
              prevTime=&prevTimeA; prevPrevTime=&prevPrevTimeA; sampleCount=&sampleCountA; tracking=&capacityTrackingA; break;
    case 'B': capacity=&capacityB; prevCurrent=&prevCurrentB; prevPrevCurrent=&prevPrevCurrentB;
              prevTime=&prevTimeB; prevPrevTime=&prevPrevTimeB; sampleCount=&sampleCountB; tracking=&capacityTrackingB; break;
    case 'C': capacity=&capacityC; prevCurrent=&prevCurrentC; prevPrevCurrent=&prevPrevCurrentC;
              prevTime=&prevTimeC; prevPrevTime=&prevPrevTimeC; sampleCount=&sampleCountC; tracking=&capacityTrackingC; break;
    case 'D': capacity=&capacityD; prevCurrent=&prevCurrentD; prevPrevCurrent=&prevPrevCurrentD;
              prevTime=&prevTimeD; prevPrevTime=&prevPrevTimeD; sampleCount=&sampleCountD; tracking=&capacityTrackingD; break;
    default: return;
  }

  bool shouldTrack = false;
  switch (slot) {
    case 'A': shouldTrack = (Acharge || Adischarge); break;
    case 'B': shouldTrack = (Bcharge || Bdischarge); break;
    case 'C': shouldTrack = (Ccharge || Cdischarge); break;
    case 'D': shouldTrack = (Dcharge || Ddischarge); break;
  }

  if (shouldTrack && !*tracking) {
    *tracking = true; *sampleCount = 0; *capacity = 0; *prevTime = 0; *prevPrevTime = 0;
    Serial.print("Slot "); Serial.print(slot); Serial.println(" capacity tracking started");
  }
  if (!shouldTrack && *tracking) {
    *tracking = false;
    Serial.print("Slot "); Serial.print(slot); Serial.print(" capacity ended: ");
    Serial.print(*capacity, 4); Serial.println(" Ah");
  }
  if (!shouldTrack) return;

  if (*sampleCount == 0) { *prevPrevTime=currentTime; *prevPrevCurrent=current; *sampleCount=1; return; }
  if (*sampleCount == 1) { *prevTime=currentTime;     *prevCurrent=current;     *sampleCount=2; return; }
  ...
  *prevPrevTime=currentTime; *prevPrevCurrent=current; *sampleCount=1;
}

void resetCapacity(char slot) {
  switch (slot) {
    case 'A': capacityA=0; sampleCountA=0; capacityTrackingA=false;
              prevCurrentA=0; prevPrevCurrentA=0; prevTimeA=0; prevPrevTimeA=0; break;
    case 'B': capacityB=0; sampleCountB=0; capacityTrackingB=false;
              prevCurrentB=0; prevPrevCurrentB=0; prevTimeB=0; prevPrevTimeB=0; break;
    case 'C': capacityC=0; sampleCountC=0; capacityTrackingC=false;
              prevCurrentC=0; prevPrevCurrentC=0; prevTimeC=0; prevPrevTimeC=0; break;
    case 'D': capacityD=0; sampleCountD=0; capacityTrackingD=false;
              prevCurrentD=0; prevPrevCurrentD=0; prevTimeD=0; prevPrevTimeD=0; break;
  }
}

void resetSlotBooleans(char slot) {
  switch (slot) {
    case 'A': Acharge=false; Adischarge=false; Acycle=false;
              AcyclePauseWait=false; AcycleRechargePauseWait=false; resetCapacity('A'); break;
    case 'B': Bcharge=false; Bdischarge=false; Bcycle=false;
              BcyclePauseWait=false; BcycleRechargePauseWait=false; resetCapacity('B'); break;
    case 'C': Ccharge=false; Cdischarge=false; Ccycle=false;
              CcyclePauseWait=false; CcycleRechargePauseWait=false; resetCapacity('C'); break;
    case 'D': Dcharge=false; Ddischarge=false; Dcycle=false;
              DcyclePauseWait=false; DcycleRechargePauseWait=false; resetCapacity('D'); break;
  }
}
...
void loop() {
  ...
  if (now - previousStatusTime >= statusInterval) {
    ...
    if (previousSlot=='\0' && !inSettingScreen && !inStopPrompt &&
        !stopMessageActive && !inBattNumberInput && !inCycleInput && !confirmingCycle) {
      lcd.setCursor(0,3);
  ...
  if (now-prevA >= interval) { prevA=now; readSensor(inaA,'A'); }
  if (now-prevB >= interval) { prevB=now; readSensor(inaB,'B'); }
  if (now-prevC >= interval) { prevC=now; readSensor(inaC,'C'); }
  if (now-prevD >= interval) { prevD=now; readSensor(inaD,'D'); }
  ...
  if (WiFi.status() != WL_CONNECTED) { if (wasConnected) Serial.println("WiFi LOST!"); wasConnected=false; }
  else                                { if (!wasConnected) Serial.println("WiFi Connected!"); wasConnected=true; }

  if (!gsheetReadyOnce && (now % 30000UL) < 200) {
    if (GSheet.ready()) { Serial.println("GSheet Ready!"); gsheetReadyOnce=true; }
  }

  // --- 60-second interval logging ---
  bool Aactive = ((Acharge||Adischarge) && battNumA>0);
  static bool lastAactive = false;
  if (Aactive && !lastAactive) { lastLogA=now; elapsedA=0; }
  lastAactive = Aactive;
  if (Aactive && (now-lastLogA >= logInterval)) {
    lastLogA=now; elapsedA+=60;
    if (voltageA <= 2.0) elapsedA=0;
    if (GSheet.ready()) {
      String statusText = Acycle ? (Acharge ? "CYCLE - CHARGING" : "CYCLE - DISCHARGING") : (Acharge ? "CHARGING" : "DISCHARGING");
      logSlotToSheet(SHEET_ID_A, statusText, battNumA, elapsedA, voltageA, currentA, capacityA);
    }
  }

  bool Bactive = ((Bcharge||Bdischarge) && battNumB>0);
  static bool lastBactive = false;
  if (Bactive && !lastBactive) { lastLogB=now; elapsedB=0; }
  lastBactive = Bactive;
  if (Bactive && (now-lastLogB >= logInterval)) {
    lastLogB=now; elapsedB+=60;
    if (voltageB <= 2.0) elapsedB=0;
    if (GSheet.ready()) {
      String statusText = Bcycle ? (Bcharge ? "CYCLE - CHARGING" : "CYCLE - DISCHARGING") : (Bcharge ? "CHARGING" : "DISCHARGING");
      logSlotToSheet(SHEET_ID_B, statusText, battNumB, elapsedB, voltageB, currentB, capacityB);
    }
  }

  bool Cactive = ((Ccharge||Cdischarge) && battNumC>0);
  static bool lastCactive = false;
  if (Cactive && !lastCactive) { lastLogC=now; elapsedC=0; }
  lastCactive = Cactive;
  if (Cactive && (now-lastLogC >= logInterval)) {
    lastLogC=now; elapsedC+=60;
    if (voltageC <= 2.0) elapsedC=0;
    if (GSheet.ready()) {
      String statusText = Ccycle ? (Ccharge ? "CYCLE - CHARGING" : "CYCLE - DISCHARGING") : (Ccharge ? "CHARGING" : "DISCHARGING");
      logSlotToSheet(SHEET_ID_C, statusText, battNumC, elapsedC, voltageC, currentC, capacityC);
    }
  }

  bool Dactive = ((Dcharge||Ddischarge) && battNumD>0);
  static bool lastDactive = false;
  if (Dactive && !lastDactive) { lastLogD=now; elapsedD=0; }
  lastDactive = Dactive;
  if (Dactive && (now-lastLogD >= logInterval)) {
    lastLogD=now; elapsedD+=60;
    if (voltageD <= 2.0) elapsedD=0;
    if (GSheet.ready()) {
      String statusText = Dcycle ? (Dcharge ? "CYCLE - CHARGING" : "CYCLE - DISCHARGING") : (Dcharge ? "CHARGING" : "DISCHARGING");
      logSlotToSheet(SHEET_ID_D, statusText, battNumD, elapsedD, voltageD, currentD, capacityD);
    }
  }
  ...
  if (inBattNumberInput) { handleBattNumberInputKey(key); return; }
  if (inCycleInput)      { handleCycleInputKey(key);      return; }

  if (confirmingCycle) {
    if (key == '#') {
      int value = pendingCycleValue;
      switch (cycleSlot) {
        case 'A': cycleTargetA=value; cycleCountA=0; resetCapacity('A'); break;
        case 'B': cycleTargetB=value; cycleCountB=0; resetCapacity('B'); break;
        case 'C': cycleTargetC=value; cycleCountC=0; resetCapacity('C'); break;
        case 'D': cycleTargetD=value; cycleCountD=0; resetCapacity('D'); break;
      }
      lcd.clear(); lcd.setCursor(0,1); lcd.print("Starting Cycle Test");
      lcd.setCursor(0,2); lcd.print("Slot "); lcd.print(cycleSlot);
      lcd.print(" - "); lcd.print(value); lcd.print(" cycles");
      ...
      lcd.clear(); lcd.setCursor(0,1); lcd.print("Cycle test CANCELLED");
      delay(1500);
      confirmingCycle=false; cycleSlot='\0'; cycleBuffer=""; pendingCycleValue=0;
  ...
  if (inStopPrompt) {
    if      (key=='0') { stopOperation(previousSlot, true); return; }
    else if (key=='5') { inStopPrompt=false; stopMessageActive=false; refreshSlotDisplay(); return; }
    return;
  }
  ...
  if (inSettingScreen) {
    if (key=='1') {
      if (battNumForSlot(previousSlot)>0) {
        ...
        lcd.clear(); lcd.setCursor(0,1); lcd.print(" Set batt# first!"); delay(1500); showSlotSetting(previousSlot);
      }
      return;
    } else if (key=='2') {
      if (battNumForSlot(previousSlot)>0) {
        resetCapacity(previousSlot); startDischargeWithDelay(previousSlot);
      ...
      showBattNumberInput(previousSlot); inBattNumberInput=true; inSettingScreen=false; return;
  ...
  switch (key) {
    case '0':
      if (previousSlot!='\0') {
        bool isActive=false;
        switch(previousSlot) {
          case 'A': isActive=(Acharge||Adischarge||Acycle||AcyclePauseWait||AcycleRechargePauseWait); break;
          case 'B': isActive=(Bcharge||Bdischarge||Bcycle||BcyclePauseWait||BcycleRechargePauseWait); break;
          case 'C': isActive=(Ccharge||Cdischarge||Ccycle||CcyclePauseWait||CcycleRechargePauseWait); break;
          case 'D': isActive=(Dcharge||Ddischarge||Dcycle||DcyclePauseWait||DcycleRechargePauseWait); break;
        }
        if (isActive) { showForceStopPrompt(previousSlot); inStopPrompt=true; }
        else {
          lcd.clear(); lcd.setCursor(0,1); lcd.print("No active operation");
          lcd.setCursor(0,2); lcd.print("in Slot "); lcd.print(previousSlot);
          delay(1500); showSlot(previousSlot);
        }
      }
      break;
    case '#':
      showHome(); previousSlot='\0'; inSettingScreen=false; inStopPrompt=false;
      inBattNumberInput=false; inCycleInput=false; confirmingCycle=false;
      break;
    ...
    case '*':
      if (previousSlot!='\0') { showSlotSetting(previousSlot); inSettingScreen=true; }
      break;
  }
}
...
// ===================== HELPERS =====================
int battNumForSlot(char slot) {
  switch(slot){ case 'A':return battNumA; case 'B':return battNumB; case 'C':return battNumC; case 'D':return battNumD; default:return 0; }
}

void startDischargeWithDelay(char slot) {
  switch(slot) {
    case 'A': Acycle=false;Acharge=false;elapsedA=0;AdischargePending=true;AdischargeWait=true;AdischargeStart=millis(); break;
    case 'B': Bcycle=false;Bcharge=false;elapsedB=0;BdischargePending=true;BdischargeWait=true;BdischargeStart=millis(); break;
    case 'C': Ccycle=false;Ccharge=false;elapsedC=0;CdischargePending=true;CdischargeWait=true;CdischargeStart=millis(); break;
    case 'D': Dcycle=false;Dcharge=false;elapsedD=0;DdischargePending=true;DdischargeWait=true;DdischargeStart=millis(); break;
  }
}
...
void startCycle(char slot) {
  switch(slot) {
    case 'A':
      Acycle=true; cycleCountA=0; resetCapacity('A'); resetSimpsonTracking('A');
      ...
      Acharge=true; digitalWrite(AMS,LOW); digitalWrite(ACD,HIGH);
      elapsedA=0; lastLogA=millis();
      logPhaseStart('A', "CYCLE - CHARGING");
      Serial.print("Slot A cycle started - Charging ("); Serial.print(cycleTargetA); Serial.println(" cycles)");
      break;
    case 'B':
      Bcycle=true; cycleCountB=0; resetCapacity('B'); resetSimpsonTracking('B');
      ...
      Bcharge=true; digitalWrite(BMS,LOW); digitalWrite(BCD,HIGH);
      elapsedB=0; lastLogB=millis();
      logPhaseStart('B', "CYCLE - CHARGING");
      Serial.print("Slot B cycle started - Charging ("); Serial.print(cycleTargetB); Serial.println(" cycles)");
      break;
    case 'C':
      Ccycle=true; cycleCountC=0; resetCapacity('C'); resetSimpsonTracking('C');
      ...
      Ccharge=true; digitalWrite(CMS,LOW); digitalWrite(CCD,HIGH);
      elapsedC=0; lastLogC=millis();
      logPhaseStart('C', "CYCLE - CHARGING");
      Serial.print("Slot C cycle started - Charging ("); Serial.print(cycleTargetC); Serial.println(" cycles)");
      break;
    case 'D':
      Dcycle=true; cycleCountD=0; resetCapacity('D'); resetSimpsonTracking('D');
      ...
      Dcharge=true; digitalWrite(DMS,LOW); digitalWrite(DCD,HIGH);
      elapsedD=0; lastLogD=millis();
      logPhaseStart('D', "CYCLE - CHARGING");
      Serial.print("Slot D cycle started - Charging ("); Serial.print(cycleTargetD); Serial.println(" cycles)");
      break;
  }
}
...
void setupSensor(INA226 &sensor, const char* label, uint8_t address) {
  if (!sensor.begin(address)) {
    Serial.print("Sensor "); Serial.print(label); Serial.println(" not detected!");
    return;
  }
  ...
  Serial.print("Sensor "); Serial.print(label); Serial.println(" initialized.");
}
...
void readSensor(INA226 &sensor, char label) {
  ...
  if (v==0.0 && c==0.0) status = "No Battery";
    ...
    switch(label) {
      case 'A': status=(cycleCompleteA&&capacityA>0)?String("Simpson Capacity ")+String(capacityA,2)+String("mAh"):"Standby"; break;
      case 'B': status=(cycleCompleteB&&capacityB>0)?String("Simpson Capacity ")+String(capacityB,2)+String("mAh"):"Standby"; break;
      case 'C': status=(cycleCompleteC&&capacityC>0)?String("Simpson Capacity ")+String(capacityC,2)+String("mAh"):"Standby"; break;
      case 'D': status=(cycleCompleteD&&capacityD>0)?String("Simpson Capacity ")+String(capacityD,2)+String("mAh"):"Standby"; break;
      default:  status="Standby"; break;
    }
  ...
  Serial.print(" | BattNo="); Serial.print(bn);
  Serial.print(" | V="); Serial.print(v,2);
  Serial.print(" | I="); Serial.print(c,0); Serial.println("mA");
  ...
  // Only calculate capacity during active charge or discharge (NOT during any pause)
        ...
        if (Acycle) {
          if (cycleCountA >= cycleTargetA) {
            ...
            if (battNumA > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_A);
              delay(100);
              logSlotToSheet(SHEET_ID_A, "Cycle Complete", battNumA, elapsedA, voltageA, 0, capacityA);
            }
            Serial.print("Slot A CYCLE COMPLETE. Final: "); Serial.print(capacityA,4); Serial.println(" Ah");
            Acycle = false;
            resetSlotRelays('A');
          } else {
            if (battNumA > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_A);
              delay(100);
              logSlotToSheet(SHEET_ID_A, "Cycle - Pause", battNumA, elapsedA, voltageA, 0, capacityA);
              delay(100);
              logSeparatorRow(SHEET_ID_A);
            }
        ...
        if (Bcycle) {
          if (cycleCountB >= cycleTargetB) {
            ...
            if (battNumB > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_B);
              delay(100);
              logSlotToSheet(SHEET_ID_B, "Cycle Complete", battNumB, elapsedB, voltageB, 0, capacityB);
            }
            Serial.print("Slot B CYCLE COMPLETE. Final: "); Serial.print(capacityB,4); Serial.println(" Ah");
            Bcycle = false;
            resetSlotRelays('B');
          } else {
            if (battNumB > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_B);
              delay(100);
              logSlotToSheet(SHEET_ID_B, "Cycle - Pause", battNumB, elapsedB, voltageB, 0, capacityB);
              delay(100);
              logSeparatorRow(SHEET_ID_B);
            }
        ...
        if (Ccycle) {
          if (cycleCountC >= cycleTargetC) {
            ...
            if (battNumC > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_C);
              delay(100);
              logSlotToSheet(SHEET_ID_C, "Cycle Complete", battNumC, elapsedC, voltageC, 0, capacityC);
            }
            Serial.print("Slot C CYCLE COMPLETE. Final: "); Serial.print(capacityC,4); Serial.println(" Ah");
            Ccycle = false;
            resetSlotRelays('C');
          } else {
            if (battNumC > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_C);
              delay(100);
              logSlotToSheet(SHEET_ID_C, "Cycle - Pause", battNumC, elapsedC, voltageC, 0, capacityC);
              delay(100);
              logSeparatorRow(SHEET_ID_C);
            }
        ...
        if (Dcycle) {
          if (cycleCountD >= cycleTargetD) {
            ...
            if (battNumD > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_D);
              delay(100);
              logSlotToSheet(SHEET_ID_D, "Cycle Complete", battNumD, elapsedD, voltageD, 0, capacityD);
            }
            Serial.print("Slot D CYCLE COMPLETE. Final: "); Serial.print(capacityD,4); Serial.println(" Ah");
            Dcycle = false;
            resetSlotRelays('D');
          } else {
            if (battNumD > 0 && GSheet.ready()) {
              ...
              logSeparatorRow(SHEET_ID_D);
              delay(100);
              logSlotToSheet(SHEET_ID_D, "Cycle - Pause", battNumD, elapsedD, voltageD, 0, capacityD);
              delay(100);
              logSeparatorRow(SHEET_ID_D);
            }
...
void processDeferredDischarge() {
  ...
  if (AcyclePauseWait && now - AcyclePauseStart >= CYCLE_PAUSE_MS) {
    ...
    Adischarge = true; elapsedA = 0; lastLogA = now;
    resetCapacity('A'); resetSimpsonTracking('A');
    digitalWrite(AMS,LOW); digitalWrite(ACD,LOW);
  ...
  if (BcyclePauseWait && now - BcyclePauseStart >= CYCLE_PAUSE_MS) {
    ...
    Bdischarge = true; elapsedB = 0; lastLogB = now;
    resetCapacity('B'); resetSimpsonTracking('B');
    digitalWrite(BMS,LOW); digitalWrite(BCD,LOW);
  ...
  if (CcyclePauseWait && now - CcyclePauseStart >= CYCLE_PAUSE_MS) {
    ...
    Cdischarge = true; elapsedC = 0; lastLogC = now;
    resetCapacity('C'); resetSimpsonTracking('C');
    digitalWrite(CMS,LOW); digitalWrite(CCD,LOW);
  ...
  if (DcyclePauseWait && now - DcyclePauseStart >= CYCLE_PAUSE_MS) {
    ...
    Ddischarge = true; elapsedD = 0; lastLogD = now;
    resetCapacity('D'); resetSimpsonTracking('D');
    digitalWrite(DMS,LOW); digitalWrite(DCD,LOW);
  ...
  if (AcycleRechargePauseWait && now - AcycleRechargePauseStart >= CYCLE_PAUSE_MS) {
    AcycleRechargePauseWait = false;
    Acharge = true; elapsedA = 0; lastLogA = now;
    digitalWrite(AMS,LOW); digitalWrite(ACD,HIGH);
    logPhaseStart('A', "CYCLE - CHARGING");
    Serial.print("Slot A cycle "); Serial.print(cycleCountA); Serial.print("/");
    Serial.print(cycleTargetA); Serial.println(" → CYCLE CHARGING");
    showSlot('A');
  }
  if (BcycleRechargePauseWait && now - BcycleRechargePauseStart >= CYCLE_PAUSE_MS) {
    BcycleRechargePauseWait = false;
    Bcharge = true; elapsedB = 0; lastLogB = now;
    digitalWrite(BMS,LOW); digitalWrite(BCD,HIGH);
    logPhaseStart('B', "CYCLE - CHARGING");
    Serial.print("Slot B cycle "); Serial.print(cycleCountB); Serial.print("/");
    Serial.print(cycleTargetB); Serial.println(" → CYCLE CHARGING");
    showSlot('B');
  }
  if (CcycleRechargePauseWait && now - CcycleRechargePauseStart >= CYCLE_PAUSE_MS) {
    CcycleRechargePauseWait = false;
    Ccharge = true; elapsedC = 0; lastLogC = now;
    digitalWrite(CMS,LOW); digitalWrite(CCD,HIGH);
    logPhaseStart('C', "CYCLE - CHARGING");
    Serial.print("Slot C cycle "); Serial.print(cycleCountC); Serial.print("/");
    Serial.print(cycleTargetC); Serial.println(" → CYCLE CHARGING");
    showSlot('C');
  }
  if (DcycleRechargePauseWait && now - DcycleRechargePauseStart >= CYCLE_PAUSE_MS) {
    DcycleRechargePauseWait = false;
    Dcharge = true; elapsedD = 0; lastLogD = now;
    digitalWrite(DMS,LOW); digitalWrite(DCD,HIGH);
    logPhaseStart('D', "CYCLE - CHARGING");
    Serial.print("Slot D cycle "); Serial.print(cycleCountD); Serial.print("/");
    Serial.print(cycleTargetD); Serial.println(" → CYCLE CHARGING");
    showSlot('D');
  }
}
...
void resetSlotRelays(char slot) {
  switch(slot) {
    case 'A': digitalWrite(AMS,HIGH);digitalWrite(ACD,HIGH); break;
    case 'B': digitalWrite(BMS,HIGH);digitalWrite(BCD,HIGH); break;
    case 'C': digitalWrite(CMS,HIGH);digitalWrite(CCD,HIGH); break;
    case 'D': digitalWrite(DMS,HIGH);digitalWrite(DCD,HIGH); break;
  }
}

void printCentered(const char* text, int row) {
  int len=strlen(text); int col=(20-len)/2; if(col<0) col=0;
...
void showSlot(char slot) {
  inSettingScreen=false; inStopPrompt=false; stopMessageActive=false;
  inBattNumberInput=false; inCycleInput=false; confirmingCycle=false;
  lcd.clear();
  float v,c,cap; String s; int bn=0;
  String mode="", cycleInfo="";

  switch(slot) {
    case 'A': v=voltageA;c=currentA;s=statusA;bn=battNumA;cap=capacityA;
      if      (AcyclePauseWait||AcycleRechargePauseWait) mode="CYCLE-PAUSE";
      else if (Acharge)         mode="CHARGING";
      else if (Adischarge)      mode="DISCHARGING";
      else if (Acycle)          mode="CYCLE";
      else                      mode="IDLE";
      if (Acycle) cycleInfo="Cyc:"+String(cycleCountA)+"/"+String(cycleTargetA);
      break;
    case 'B': v=voltageB;c=currentB;s=statusB;bn=battNumB;cap=capacityB;
      if      (BcyclePauseWait||BcycleRechargePauseWait) mode="CYCLE-PAUSE";
      else if (Bcharge)         mode="CHARGING";
      else if (Bdischarge)      mode="DISCHARGING";
      else if (Bcycle)          mode="CYCLE";
      else                      mode="IDLE";
      if (Bcycle) cycleInfo="Cyc:"+String(cycleCountB)+"/"+String(cycleTargetB);
      break;
    case 'C': v=voltageC;c=currentC;s=statusC;bn=battNumC;cap=capacityC;
      if      (CcyclePauseWait||CcycleRechargePauseWait) mode="CYCLE-PAUSE";
      else if (Ccharge)         mode="CHARGING";
      else if (Cdischarge)      mode="DISCHARGING";
      else if (Ccycle)          mode="CYCLE";
      else                      mode="IDLE";
      if (Ccycle) cycleInfo="Cyc:"+String(cycleCountC)+"/"+String(cycleTargetC);
      break;
    case 'D': v=voltageD;c=currentD;s=statusD;bn=battNumD;cap=capacityD;
      if      (DcyclePauseWait||DcycleRechargePauseWait) mode="CYCLE-PAUSE";
      else if (Dcharge)         mode="CHARGING";
      else if (Ddischarge)      mode="DISCHARGING";
      else if (Dcycle)          mode="CYCLE";
      else                      mode="IDLE";
      if (Dcycle) cycleInfo="Cyc:"+String(cycleCountD)+"/"+String(cycleTargetD);
      break;
  }

  String title="Slot "; title+=slot;
  if (bn>0){ title+=" #"; title+=bn; }
  ...
  if (Adischarge||Bdischarge||Cdischarge||Ddischarge) {
    lcd.setCursor(0,3); lcd.print("Cap:"); lcd.print(cap,0); lcd.print("mAh V:"); lcd.print(v,2);
  } else {
...
void showForceStopPrompt(char slot) {
  ...
  switch(slot) {
    case 'A': if(AcyclePauseWait||AcycleRechargePauseWait) lcd.print("PAUSING");
              else if(Acharge) lcd.print("CHARGING");
              else if(Adischarge) lcd.print("DISCHARGING");
              else if(Acycle) lcd.print("CYCLE"); break;
    case 'B': if(BcyclePauseWait||BcycleRechargePauseWait) lcd.print("PAUSING");
              else if(Bcharge) lcd.print("CHARGING");
              else if(Bdischarge) lcd.print("DISCHARGING");
              else if(Bcycle) lcd.print("CYCLE"); break;
    case 'C': if(CcyclePauseWait||CcycleRechargePauseWait) lcd.print("PAUSING");
              else if(Ccharge) lcd.print("CHARGING");
              else if(Cdischarge) lcd.print("DISCHARGING");
              else if(Ccycle) lcd.print("CYCLE"); break;
    case 'D': if(DcyclePauseWait||DcycleRechargePauseWait) lcd.print("PAUSING");
              else if(Dcharge) lcd.print("CHARGING");
              else if(Ddischarge) lcd.print("DISCHARGING");
              else if(Dcycle) lcd.print("CYCLE"); break;
  }
...
void showBattNumberInput(char slot) {
  ...
  lcd.setCursor(0,3); lcd.print(battNumForSlot(slot));
}
...
void renderCycleBuffer() {
  String d="["; d+=cycleBuffer; d+="]";
  int col=(20-d.length())/2; if(col<0) col=0;
  lcd.setCursor(0,1); for(int i=0;i<20;i++) lcd.print(" ");
  lcd.setCursor(col,1); lcd.print(d);
}
...
void handleCycleInputKey(char key) {
  if (key>='0' && key<='9') {
    ...
    if (cycleBuffer.length()<3) { cycleBuffer+=key; renderCycleBuffer(); }
    return;
  }
  if (key=='*') {
    inCycleInput=false; confirmingCycle=false; cycleSlot='\0'; cycleBuffer=""; pendingCycleValue=0;
    showSlotSetting(previousSlot); inSettingScreen=true; return;
  }
  if (key=='#') {
...
void handleBattNumberInputKey(char key) {
  if (key>='0' && key<='9') {
    ...
    if (battNumberBuffer.length()<3) { battNumberBuffer+=key; renderBattNumberBuffer(); }
    return;
  }
  if (key=='*') {
  ...
  if (key=='#') {
    int value=(battNumberBuffer.length()>0)?atoi(battNumberBuffer.c_str()):0;
    saveBattNum(battNumberSlot, value);
    lcd.clear(); lcd.setCursor(0,1); lcd.print(" Batt # saved!");
    lcd.setCursor(0,2); lcd.print(" Slot "); lcd.print(battNumberSlot); lcd.print(" = "); lcd.print(value);
    delay(1500);
    inBattNumberInput=false; battNumberSlot='\0'; battNumberBuffer="";
    showSlotSetting(previousSlot); inSettingScreen=true;
  }
...
void saveBattNum(char slot, int num) {
  switch(slot) {
    case 'A': prefs.putInt("A_num",num); battNumA=num; break;
    case 'B': prefs.putInt("B_num",num); battNumB=num; break;
    case 'C': prefs.putInt("C_num",num); battNumC=num; break;
    case 'D': prefs.putInt("D_num",num); battNumD=num; break;
  }
  Serial.print("Saved "); Serial.print(slot); Serial.print("_num="); Serial.println(num);
}

void clearBattNum(char slot) {
  switch(slot) {
    case 'A': prefs.putInt("A_num",0); battNumA=0; break;
    case 'B': prefs.putInt("B_num",0); battNumB=0; break;
    case 'C': prefs.putInt("C_num",0); battNumC=0; break;
    case 'D': prefs.putInt("D_num",0); battNumD=0; break;
  }
  Serial.print("Cleared "); Serial.print(slot); Serial.println(" number");
}

void stopOperation(char slot, bool fromForceStop) {
  String operation="";
  switch(slot) {
    case 'A': if(AcyclePauseWait||AcycleRechargePauseWait||Acycle) operation="CYCLE";
              else if(Acharge) operation="CHARGING";
              else if(Adischarge) operation="DISCHARGING"; break;
    case 'B': if(BcyclePauseWait||BcycleRechargePauseWait||Bcycle) operation="CYCLE";
              else if(Bcharge) operation="CHARGING";
              else if(Bdischarge) operation="DISCHARGING"; break;
    case 'C': if(CcyclePauseWait||CcycleRechargePauseWait||Ccycle) operation="CYCLE";
              else if(Ccharge) operation="CHARGING";
              else if(Cdischarge) operation="DISCHARGING"; break;
    case 'D': if(DcyclePauseWait||DcycleRechargePauseWait||Dcycle) operation="CYCLE";
              else if(Dcharge) operation="CHARGING";
              else if(Ddischarge) operation="DISCHARGING"; break;
  }
  float finalCapacity=0;
  switch(slot){ case 'A':finalCapacity=capacityA;break; case 'B':finalCapacity=capacityB;break;
                case 'C':finalCapacity=capacityC;break; case 'D':finalCapacity=capacityD;break; }

  resetSlotBooleans(slot);
  resetSlotRelays(slot);

  char sheetId[50]; float v=0,c=0; int battNum=battNumForSlot(slot);
  switch(slot){
  ...
  if (battNum>0) logSlotToSheet(sheetId, "Force Stopped", battNum, 0, v, c, finalCapacity);
  ...
  lcd.setCursor(0,3); lcd.print("Slot "); lcd.print(slot);
  if (operation.length()>0){ lcd.print(" - "); lcd.print(operation); }

  stopMessageStart=millis(); stopMessageActive=true; inStopPrompt=false;
  Serial.print("Slot "); Serial.print(slot); Serial.print(" stopped. Was: "); Serial.println(operation);
}
...
void logSimpsonOutputRow(const char* sheetId, float voltage, float current, float capacity) {
  ...
  GSheet.values.append(&response, sheetId, "Sheet1!A1:G1", &valueRange);
  Serial.print("Simpson Output row logged: "); Serial.print(capacityStr); Serial.println(" Ah");
}

void logSlotToSheet(const char* sheetId, const String& slotStatus, int battNum,
                    unsigned long elapsedSecs, float voltage, float current,
                    float capacity) {
  if (!GSheet.ready()) { Serial.println("GSheet not ready - skip"); return; }
  FirebaseJson response, valueRange;
  char timeStamp[25]; formatDateTime("%Y-%m-%d %H:%M:%S", timeStamp, sizeof(timeStamp));
  ...
  Serial.print("Sheet log → "); Serial.print(sheetId);
  Serial.print(" | "); Serial.print(slotStatus);
  Serial.print(" | cap="); Serial.print(capacity * 1000.0, 4); Serial.println(" mAh");

  bool ok = GSheet.values.append(&response, sheetId, "Sheet1!A1:G1", &valueRange);
  if (!ok) {
    Serial.println("Append FAILED!");
    String r; response.toString(r,true); Serial.println(r);
  } else {
